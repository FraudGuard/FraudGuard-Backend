...
export const evaluateExclusions = async (resultingAd: AdsSchema) => {
    ...
    if (resultingAd.ap_titel_enthaelt_gebraucht === 1) { ausschlusskriterium_erfuellt = 1; }
    if (resultingAd.ap_titel_enthaelt_suche === 1) { ausschlusskriterium_erfuellt = 1; }
    if (resultingAd.ap_titel_enthaelt_tausche === 1) { ausschlusskriterium_erfuellt = 1; }
    if (resultingAd.ap_titel_enthaelt_kilo === 1) { ausschlusskriterium_erfuellt = 1; }
    if (resultingAd.ap_titel_enthaelt_sammlung === 1) { ausschlusskriterium_erfuellt = 1; }


    if (resultingAd.ap_beschreibung_enthaelt_tausch === 1) { ausschlusskriterium_erfuellt = 1; }
    if (resultingAd.ap_beschreibung_enthaelt_suche === 1) { ausschlusskriterium_erfuellt = 1; }


    if (resultingAd.ap_preis_ist_leer === 1) { ausschlusskriterium_erfuellt = 1; }


    if (resultingAd.ap_sonstiges_anzeige_suche === 1) { ausschlusskriterium_erfuellt = 1; }
