...
export const analyzeKonto = async (
  ...
  // }
  const foundSpecialChars = ad['contact-name'].value.match(/[@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/g);
  resultingAd.konto_name_sonderzeichen_anzahl = foundSpecialChars?.length ?? 0

  resultingAd.konto_erstellt_timestamp = new Date(ad['user-since-date-time'].value).getTime()

  resultingAd.konto_name_natuerlich = firstNames.find(x => ad['contact-name'].value.toLocaleLowerCase().includes(x.toLocaleLowerCase())) ? 1 : 0
  resultingAd.konto_name_enthaelt_unueblich = countries.find(x => ad['contact-name'].value.toLocaleLowerCase().includes(x.name.toLocaleLowerCase())) ? 1 : 0


  if (adsFromAccount) {
    resultingAd.konto_anzeigen_anzahl = adsFromAccount.length
    resultingAd.konto_anzeigen_ueber_100 = adsFromAccount.filter(x => x.price.amount.value > 100).length

    resultingAd.konto_anzeigen_gleich = 0;
    for (const adFromAccount of adsFromAccount) {
      if (
        adFromAccount.title.value.includes(ad.title.value.toLocaleLowerCase())
        && adFromAccount.description.value.includes(ad.description.value.toLocaleLowerCase())
      ) {
