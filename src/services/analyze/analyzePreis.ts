import { AdsFromEbaySchema } from '../../api/models';
import { logger } from '../../shared';

export const analyzePreis = async (ad: AdsFromEbaySchema) => {
  logger.info('start analyze Preis');

  // Abgleich mit Durchschnittswert des Produkts auf eBay WIP
  // const produkt_aus_titel = ad.title.value;

  // if (produkt_aus_titel.includes('lego')){
  // const num = produkt_aus_titel.replace(/^\D+|\D+$/g, "");}

  // Prozentwert der Abweichung von Amount und Marktwert WIP

  const preis_unter_marktwert = ad.price?.amount?.value;
  const preis_abweichung_marktwert = ad.price?.amount?.value;
  const preis_waehrung_eur =
    ad.price['currency-iso-code'].value === 'EUR' ? 0 : 1;
  const preis_typ_vb =
    ad.price['price-type'].value === 'PLEASE_CONTACT' ? 0 : 1;
  const ap_preis_ist_leer = ad.price?.amount?.value === 0 ? 0 : 1;

  const resultingAd = {
    preis_unter_marktwert,
    preis_abweichung_marktwert,
    preis_waehrung_eur,
    preis_typ_vb,
    ap_preis_ist_leer,
  };

  return resultingAd;
};
