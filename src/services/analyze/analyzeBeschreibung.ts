...
export const analyzeBeschreibung = (
  ...
  new Promise((resolve, reject) => {
    ...
    for ( var signalwort of ueberweisung_signalwoerter){
        ...
        if (beschreibung.includes(signalwort)  && !beschreibung.includes(ueberweisung_signalwoerter_gegenteil[i])){ enthaelt_signalwort = 1}
        i += 1
    }
    ...
    for ( var signalwort of versand_signalwoerter){
        var beschreibung = ad['description']?.value?.toLowerCase()
        if (beschreibung.includes(signalwort)  && !beschreibung.includes(versand_signalwoerter_gegenteil[i])){ enthaelt_signalwort = 1}
        i += 1
    }
    ...
    var neu_signalwoerter_gegenteil = ["nicht neu", "nicht new", "nicht unbenutzt", "nicht nie ausgepackt", "nicht ungeoeffnet", "nicht nicht benutzt"];
    enthaelt_signalwort = 0
    i = 0
    for ( var signalwort of neu_signalwoerter){
        var beschreibung = ad['description']?.value?.toLowerCase()
        if (beschreibung.includes(signalwort)  && !beschreibung.includes(neu_signalwoerter_gegenteil[i])){ enthaelt_signalwort = 1}
        i += 1
    }
    ...
    for ( var signalwort of ovp_signalwoerter){
        var beschreibung = ad['description']?.value?.toLowerCase()
        if (beschreibung.includes(signalwort)  && !beschreibung.includes(ovp_signalwoerter_gegenteil[i])){ enthaelt_signalwort = 1}
        i += 1
    }
   ...
   for ( var signalwort of versiegelt_signalwoerter){
       var beschreibung = ad['description']?.value?.toLowerCase()
       if (beschreibung.includes(signalwort)  && !beschreibung.includes(versiegelt_signalwoerter_gegenteil[i])){ enthaelt_signalwort = 1}
       i += 1
   }
  ...
  for ( var signalwort of barzahlung_signalwoerter){
      var beschreibung = ad['description']?.value?.toLowerCase()
      if (beschreibung.includes(signalwort)  && !beschreibung.includes(barzahlung_signalwoerter_gegenteil[i])){ enthaelt_signalwort = 1}
      i += 1
  }
  resultingAd.ap_beschreibung_enthaelt_barzahlung = enthaelt_signalwort

  //Prüfen, ob Artikel gebraucht ist
  var gebraucht_signalwoerter = ["gebrauchsspuren", "benutzt", "gebraucht", "aufgebaut"];
  var gebraucht_signalwoerter_gegenteil = ["keine gebrauchsspuren","nicht benutzt", "nicht gebraucht",  "nie aufgebaut"];
  enthaelt_signalwort = 0
  i = 0
  for ( var signalwort of gebraucht_signalwoerter){
      var beschreibung = ad['description']?.value?.toLowerCase()
      if (beschreibung.includes(signalwort)  && !beschreibung.includes(gebraucht_signalwoerter_gegenteil[i])){ enthaelt_signalwort = 1}
      i += 1
  }
  ...
  for ( var signalwort of tausch_signalwoerter){
      var beschreibung = ad['description']?.value?.toLowerCase()
      if (beschreibung.includes(signalwort)  && !beschreibung.includes(tausch_signalwoerter_gegenteil[i])){ enthaelt_signalwort = 1}
      i += 1
  }
  ...
  for ( var signalwort of abholung_signalwoerter){
      var beschreibung = ad['description']?.value?.toLowerCase()
      if (beschreibung.includes(signalwort)  && !beschreibung.includes(abholung_signalwoerter_gegenteil[i])){ enthaelt_signalwort = 1}
      i += 1
  }
  ...
  for ( var signalwort of suche_signalwoerter){
      var beschreibung = ad['description']?.value?.toLowerCase()
      if (beschreibung.includes(signalwort)  && !beschreibung.includes(suche_signalwoerter_gegenteil[i])){ enthaelt_signalwort = 1}
      i += 1
  }
  resultingAd.ap_beschreibung_enthaelt_suche = enthaelt_signalwort

  //Prüfen auf Sammlungsauflösung
  var sammleraufloesung_signalwoerter = ["sammlerauflösung", "sammlungsauflösung", "sammlung", "löse sammlung auf"];
  var sammleraufloesung_signalwoerter_gegenteil = ["keine sammlerauflösung", "keine sammlungsauflösung", "keine sammlung", "löse sammlung nicht auf"];
  enthaelt_signalwort = 0
  i = 0
  for ( var signalwort of sammleraufloesung_signalwoerter){
      var beschreibung = ad['description']?.value?.toLowerCase()
      if (beschreibung.includes(signalwort)  && !beschreibung.includes(sammleraufloesung_signalwoerter_gegenteil[i])){ enthaelt_signalwort = 1}
      i += 1
  }
